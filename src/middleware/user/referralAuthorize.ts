...
export const  RefrralMiddleaware=async(req:Request,res: Response,next: NextFunction)=>{
    ...
    if(!referral){
        res.status(201).json({message:"Invalid Call .Please Fill The referral"});
    }
    try {
        const referralCheck=await ReferralDb.findOne({referralCode:referral})
        if(!referralCheck){
        res.status(201).json({messgae:"Invalid Refrral"});
        }
        // const comunity=await CommunityModel.findById(communityId);
        const community = await CommunityModel.findById(communityId);
        if (!community) {
            return res.status(201).json({ message: "Community not found" });
        }
        ...
        if (community._id.toString() !== referralCheck?.communityInfo.toString()) {
            return res.status(201).json({ message: "Wrong community" });
        }
        ...
        const alreadyCompleted = task.completions?.some(
          ...
          if ( alreadyCompleted )
          {
            ...
            res.status(201).json({ message: "Task already completed by this user" });
            return;
        }
